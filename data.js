// ─────────────────────────────────────────────
//  DATA LAYER  —  swap this out for AI-generated
//  data in v2 without touching app.js or styles.css
// ─────────────────────────────────────────────

const ARCHETYPES = [
  { id: 'cash-confusion',   label: 'Cash-basis confusion',    tier: 'Tier 1', method: 'Cash',   biz: 'Freelance design studio',      files: 35 },
  { id: 'month-end',        label: 'Month-end close',         tier: 'Tier 3', method: 'Accrual',biz: 'Professional services firm',   files: 60 },
  { id: 'first-month',      label: 'First month on the job',  tier: 'Tier 2', method: 'Accrual',biz: 'E-commerce retailer (Shopify)', files: 50 },
  { id: 'growing-pains',    label: 'Growing pains',           tier: 'Tier 2', method: 'Accrual',biz: 'SaaS startup (15 employees)',   files: 55 },
  { id: 'ap-backlog',       label: 'AP backlog crisis',        tier: 'Tier 2', method: 'Accrual',biz: 'Medical practice',              files: 50 },
  { id: 'recon-nightmare',  label: 'Reconciliation nightmare', tier: 'Tier 3', method: 'Accrual',biz: 'Property management company',   files: 65 },
  { id: 'tax-prep',         label: 'Tax season prep',          tier: 'Tier 2', method: 'Cash',   biz: 'Construction contractor',       files: 45 },
  { id: 'multi-entity',     label: 'Multi-entity tangle',      tier: 'Tier 3', method: 'Accrual',biz: 'Restaurant group (3 locations)',files: 75 },
];

// ── GENERATION PROMPT ─────────────────────────

function buildGenerationPrompt(archetype) {
  return `You are generating a realistic accounting sandbox world for the Accounting APEX benchmark.

Archetype: "${archetype.label}"
Business type: ${archetype.biz}
Accounting method: ${archetype.method}
Difficulty tier: ${archetype.tier}

Generate a complete, self-contained accounting world as a JSON object. The world should feel like inheriting real books from a real business — messy, internally consistent, with embedded ambiguity.
...

{
  ...
  "transactions": [
    {
      "date": "MM/DD",
      "desc": "BANK FEED DESCRIPTION IN CAPS",
      "amount": number (positive=credit, negative=debit),
      "flag": "clear|recur|ambig|personal|dupe",
      "note": "brief explanation of why this is interesting or ambiguous"
    }
  ],
  "chartOfAccounts": [
    { "code": "1010", "name": "Account Name", "type": "Asset|Liability|Equity|Revenue|Expense" }
  ],
  "oldChartOfAccounts": [
    { "code": "1010", "name": "Old Account Name (different from current)", "type": "Asset|Liability|Equity|Revenue|Expense" }
  ],
  "expensePolicy": [
    { "key": "Policy item name", "val": "Policy description" }
  ],
  "oldExpensePolicy": [
    { "key": "Policy item name", "val": "Old value — OUTDATED explanation" }
  ],
  "invoices": [
    {
      "id": "inv_1",
  ...
  "rubric": [
    {
      "n": 1,
      "text": "Specific, binary, verifiable rubric criterion for this world's task",
      "type": "det|llm|neg",
      "label": "deterministic|llm judge|negative criterion"
    }
  ],
  "taskPrompt": "The full task prompt an agent would receive for this world.",
  "ambiguityTypes": ["list", "of", "ambiguity", "types", "embedded", "in", "this", "world"],
  "misleadingFiles": [
    { "file": "filename.ext", "why": "Why this file is misleading and what error it causes" }
  ]
}

Requirements:
- 18-28 transactions, mix of credits and debits, realistic bank feed descriptions
- Include at least: 2-3 recurring subscriptions, 1-2 ambiguous vendor names, 1 personal expense, 1 duplicate or near-duplicate
- Chart of accounts: 10-14 accounts appropriate for this business type
- Old chart of accounts: 6-8 accounts with DIFFERENT names than current (this is the trap)
- Expense policy: 4-6 items, at least one with a specific dollar threshold
- Old expense policy: 2-3 items where the old values differ meaningfully from current
- Invoices: 2-3 invoices, one should be a potential duplicate or have a warning
- Rubric: exactly 6 criteria — 4 deterministic, 1 llm judge, 1 negative criterion
- Task prompt: specific to this business, references actual file names
- Ambiguity types: 4-6 specific types embedded in this world
- Make everything internally consistent — numbers should add up, invoices should match transactions
- The old files should be genuinely misleading — different account names, different thresholds`;
}
